...
#if defined(NDEBUG)
# error "Mkcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xeccfdcab784e109e1f8a70717a4605abca019703ca2f9207ea27c16277eecd9b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Mkcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Mkcoin Signed Message:\n";
...
{
    // Mkcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 10 * 30; // Mkcoin: 3.5 days
static const int64 nTargetSpacing = 1 * 30; // Mkcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Mkcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Mkcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xeccfdcab784e109e1f8a70717a4605abca019703ca2f9207ea27c16277eecd9ts");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "06/Sep/2018 Trump sees treason from within";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1536240923;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1677637;
        ...
        assert(block.hashMerkleRoot == uint256("0xdd671a7431690615d33a354211c72e2e224a246cc70a1432afdbffcb5c3edb1cts"));
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd1, 0xbd }; // Mkcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MkcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MkcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MkcoinMiner(CWallet *pwallet)
{
    printf("MkcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mkcoin-miner");
    ...
    try { loop {
        ...
        printf("Running MkcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MkcoinMiner, pwallet));
}
